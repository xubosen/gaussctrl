...
class GaussCtrlPipelineConfig(VanillaPipelineConfig):
    ...
    datamanager: GaussCtrlDataManagerConfig = (
        field(default_factory=GaussCtrlDataManagerConfig))
    ...
    def render_reverse(self):
        ...
        for cam_idx in range(len(self.datamanager.cameras)):
            ...
            if self.config.langsam_obj != "":
                ...
                rgb_filename = os.path.join(debug_dir, f'cam_{cam_idx:03d}_langsam_input.png')
                ...
                mask_img = Image.fromarray((mask_npy * 255).astype(np.uint8))
